...
from slora.utils.infer_utils import get_profiler, nvtx_decorator
...
class RouterManager:
            ...
            if self.running_batch is not None:
                if counter_count % 50 == 0:
                    print("counter_count: ", counter_count,
                          "current batch size:", len(self.running_batch.reqs), 
                          "token used ratio:", self.running_batch.calcu_used_tokens() / self.input_params.max_total_token_num, 
    ...
    async def _prefill_batch(self, batch, minibatch=True):
        ...
        if profiler == "TORCH_PROFILER":
            ...
            with torch.profiler.profile(
                ...
                #     wait=1,
                #     warmup=1,
                #     active=2), ################### cannot detect cpu_op
                on_trace_ready=torch.profiler.tensorboard_trace_handler(result_dir, worker_name='worker0'),
                record_shapes=True,
    ...
    async def _decode_batch(self, batch:Batch):
        profiler = get_profiler()
        if profiler == "TORCH_PROFILER":
            result_dir = './profiler-traces/result_decode'
            with torch.profiler.profile(
                activities=[
                    torch.profiler.ProfilerActivity.CPU,
                    torch.profiler.ProfilerActivity.CUDA,
